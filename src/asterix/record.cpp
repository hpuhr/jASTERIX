 ...
 : ItemParser (item_definition)
{
    ...
    if (!field_specification.is_object())
        throw runtime_error ("parsing record item '"+name_+"' field specification is not an object");
    ...
    if (item_definition.find("items") == item_definition.end())
        throw runtime_error ("parsing record item '"+name_+"' without items");

    const json& items = item_definition.at("items");

    if (!items.is_array())
        throw runtime_error ("parsing record item '"+name_+"' field specification is not an array");
                              ...
                              nlohmann::json& target, bool debug)
{
    ...
    {
        if (debug)
            loginf << "parsing record item '" << name_ << "' data item '" << data_item_it->name() << "' index "
                   << index+parsed_bytes;

        parsed_bytes += data_item_it->parseItem(data, index+parsed_bytes, size, parsed_bytes, target, debug);
    }

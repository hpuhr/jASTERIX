    ...
    : ItemParserBase (item_definition)
{
    ...
    if (type_ == "ComplexSpecialPurposeField")
    {
        ...
        if (!item_definition.contains("field_specification"))
            throw runtime_error ("SpecialPurposeField item '"+name_+"' parsing without field specification");

        const json& field_specification = item_definition.at("field_specification");

        if (!field_specification.is_object())
            throw runtime_error ("SpecialPurposeField item '"+name_+"' field specification is not an object");
        ...
        if (!item_definition.contains("items_indicator"))
            throw runtime_error ("SpecialPurposeField item '"+name_+"' without items indicator definition");

        const json& items_indicator = item_definition.at("items_indicator");

        if (!items_indicator.is_array())
            throw runtime_error ("SpecialPurposeField item '"+name_+"' items indicator definition is not an array");
        ...
        if (!items.is_array())
            throw runtime_error ("SpecialPurposeField item '"+name_+"' field specification is not an array");
        ...
        for (const json& data_item_it : items)
        {
            if (!data_item_it.contains("number"))
                throw runtime_error ("SpecialPurposeField item '"+data_item_it.dump(4)+"' without number");
            ...
            if (complex_items_.count(item_number) != 0)
                throw runtime_error ("SpecialPurposeField item '"+name_+"' item number '"+item_number
                                     +"' used multiple times");
    ...
    {
        if (item_definition.contains("field_specification"))
            throw runtime_error ("SpecialPurposeField item '"+name_+"' simple but contains field specification");

        if (item_definition.contains("items_indicator"))
            throw runtime_error ("SpecialPurposeField item '"+name_+"' simple but contains items indicator definition");
        ...
        {
            if (data_item_it.contains("number"))
                throw runtime_error ("SpecialPurposeField simple but item '"+data_item_it.dump(4)+"' contains number");
...
size_t SpecialPurposeField::parseItem (const char* data, size_t index, size_t size, size_t current_parsed_bytes,
...
size_t SpecialPurposeField::parseSimpleItem (const char* data, size_t index, size_t size, size_t current_parsed_bytes,
                                       nlohmann::json& target, bool debug)
{
    if (debug)
        loginf << "parsing simple SpecialPurposeField item '" << name_ << "' with index " << index << " size " << size
    ...
    {
        if (debug)
            loginf << "parsing simple SpecialPurposeField item '" << name_ << "' data item '" << item_it->name()
                   << "' index " << index+parsed_bytes << logendl;

        parsed_bytes += item_it->parseItem(data, index+parsed_bytes, size, parsed_bytes, target, debug);
    }

    return parsed_bytes;
}

size_t SpecialPurposeField::parseComplexItem (const char* data, size_t index, size_t size, size_t current_parsed_bytes,
                                       nlohmann::json& target, bool debug)
{
    if (debug)
        loginf << "parsing complex SpecialPurposeField item '" << name_ << "' with index " << index << " size " << size
               << " current parsed bytes " << current_parsed_bytes << logendl;

    size_t parsed_bytes {0};

    if (debug)
        loginf << "parsing complex SpecialPurposeField item '"+name_+"' field specification" << logendl;

    parsed_bytes = complex_field_specification_->parseItem(data, index+parsed_bytes, size, parsed_bytes, target, debug);
    ...
    //        throw runtime_error ("SpecialPurposeField item '"+name_+"' has more FSPEC bits than defined uap items");
    ...
    for (const auto& item_name : complex_items_names_) // parse static uap items
    {
        ...
        if (fspec_bits.at(uap_cnt))
        {
            ...
            if (debug)
                loginf << "parsing complex SpecialPurposeField item '" << name_ << "' data item '" << item_name
                       << "' index " << index+parsed_bytes << logendl;

            if (complex_items_.count(item_name) != 1)
                throw runtime_error ("complex SpecialPurposeField item '"+name_+"' references undefined item '"
                                     +item_name+"'");

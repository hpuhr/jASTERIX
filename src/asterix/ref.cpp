 ...
 : ItemParserBase (item_definition)
{
    ...
    if (!field_specification.is_object())
        throw runtime_error ("ReservedExpansionField item '"+name_+"' field specification is not an object");
    ...
    if (!items_indicator.is_array())
        throw runtime_error ("ReservedExpansionField item '"+name_+"' items indicator definition is not an array");
    ...
    if (!items.is_array())
        throw runtime_error ("ReservedExpansionField item '"+name_+"' field specification is not an array");
    ...
    for (const json& data_item_it : items)
    {
        ...
        if (items_.count(item_number) != 0)
            throw runtime_error ("ReservedExpansionField item '"+name_+"' item number '"+item_number+"' used multiple times");
...
size_t ReservedExpansionField::parseItem (const char* data, size_t index, size_t size, size_t current_parsed_bytes,
                              nlohmann::json& target, bool debug)
{
    if (debug)
        loginf << "parsing ReservedExpansionField item '" << name_ << "' with index " << index << " size " << size
               << " current parsed bytes " << current_parsed_bytes << logendl;

    size_t parsed_bytes {0};

    if (debug)
        loginf << "parsing ReservedExpansionField item '"+name_+"' field specification" << logendl;
...
//    bool reserved_expansion_field_present {false};
    ...
    for (const auto& item_name : items_names_) // parse static uap items
    {
        ...
        if (fspec_bits.at(uap_cnt))
        {
            ...
            if (debug)
                loginf << "parsing ReservedExpansionField item '" << name_ << "' data item '" << item_name
                       << "' index " << index+parsed_bytes << logendl;

            if (items_.count(item_name) != 1)
                throw runtime_error ("ReservedExpansionField item '"+name_+"' references undefined item '"
                                     +item_name+"'");
...
//bool ReservedExpansionField::compareKey (const nlohmann::json& container, const std::string& value)
//{

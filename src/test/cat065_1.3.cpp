...
{
    loginf << "cat065 test: decoded " << num_frames << " frames, " << num_records << " records: " << json_data.dump(4)
               << logendl;

    loginf << "cat065 test: data block" << logendl;

    assert (json_data["data_block"]["category"] == 65);
...
//    }
    ...
    assert (json_data.at("data_block").at("content").at("records")[0].at("000").at("Message Type") == 02);
    ...
    assert (json_data.at("data_block").at("content").at("records")[0].at("020").at("Batch Number") == 0);
}
...
void test_cat065 (jASTERIX::jASTERIX& jasterix)
{
    ...
    assert (jasterix.hasCategory("065"));
    assert (jasterix.hasEdition("065", "1.3"));
    jasterix.setEdition("065", "1.3");
    jasterix.setMapping("065", "");

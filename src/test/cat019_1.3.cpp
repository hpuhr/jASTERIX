...
{
    loginf << "cat019 test: decoded " << num_frames << " frames, " << num_records << " records: " << json_data.dump(4)
               << logendl;

    loginf << "cat019 test: data block" << logendl;

    assert (json_data["data_block"]["category"] == 19);
    assert (json_data["data_block"]["length"] == 57);
...
//    }
    ...
    assert (json_data.at("data_block").at("content").at("records")[0].at("FSPEC").size() == 2*8);
    ...
    assert (json_data.at("data_block").at("content").at("records")[0].at("550").at("NOGO") == 0);
    assert (json_data.at("data_block").at("content").at("records")[0].at("550").at("OVL") == 0);
    assert (json_data.at("data_block").at("content").at("records")[0].at("550").at("TSV") == 0);
    assert (json_data.at("data_block").at("content").at("records")[0].at("550").at("TTF") == 0);

    //    ;  I019/551: =0x d0
    //    ;  Tracking Processor Detailed Status: 0xd0
    // 11010000
    loginf << "cat019 test: 551" << logendl;
    assert (json_data.at("data_block").at("content").at("records")[0].at("551").at("TP 1a") == 1);
    assert (json_data.at("data_block").at("content").at("records")[0].at("551").at("TP 1b") == 1);
    assert (json_data.at("data_block").at("content").at("records")[0].at("551").at("TP 2a") == 0);
    assert (json_data.at("data_block").at("content").at("records")[0].at("551").at("TP 2b") == 1);
    assert (json_data.at("data_block").at("content").at("records")[0].at("551").at("TP 3a") == 0);
    assert (json_data.at("data_block").at("content").at("records")[0].at("551").at("TP 3b") == 0);
    assert (json_data.at("data_block").at("content").at("records")[0].at("551").at("TP 4a") == 0);
    assert (json_data.at("data_block").at("content").at("records")[0].at("551").at("TP 4b") == 0);
    ...
    assert (json_data.at("data_block").at("content").at("records")[0].at("552").at("REP") == 16);
    assert (json_data.at("data_block").at("content").at("records")[0].at("552").at("Remote Sensor Detailed Status").size() == 16);
    // 0110 1100
    // spare 0
    // "Receiver 1090 MHz": 1,
    // "Transmitter 1030 MHz": 1,
    ...
    assert (json_data.at("data_block").at("content").at("records")[0].at("552").at("Remote Sensor Detailed Status")[0].at("Receiver 1090 MHz") == 1);
    assert (json_data.at("data_block").at("content").at("records")[0].at("552").at("Remote Sensor Detailed Status")[0].at("Transmitter 1030 MHz") == 1);
    assert (json_data.at("data_block").at("content").at("records")[0].at("552").at("Remote Sensor Detailed Status")[0].at("Transmitter 1090 MHz") == 0);
    assert (json_data.at("data_block").at("content").at("records")[0].at("552").at("Remote Sensor Detailed Status")[0].at("RS Status") == 1);
    assert (json_data.at("data_block").at("content").at("records")[0].at("552").at("Remote Sensor Detailed Status")[0].at("RS Operational") == 1);
    ...
    tmp_d = json_data.at("data_block").at("content").at("records")[0].at("600").at("Latitude");
    assert (fabs(tmp_d-47.5) < 10e-6);
    tmp_d = json_data.at("data_block").at("content").at("records")[0].at("600").at("Longitude");
    assert (fabs(tmp_d-14.0) < 10e-6);

    //    ;  I019/610: =0x 00 00
    //    ;  Height of the MLT System Reference Point: 0.00 mtr
    loginf << "cat019 test: 610" << logendl;
    assert (json_data.at("data_block").at("content").at("records")[0].at("610").at("Height") == 0.0);

    //    ;  I019/620: =0x 00
    //    ;  WGS-84 Undulation: 0 mtr
    loginf << "cat019 test: 620" << logendl;
    assert (json_data.at("data_block").at("content").at("records")[0].at("620").at("Undulation") == 0.0);
}
...
void test_cat019 (jASTERIX::jASTERIX& jasterix)
{
    ...
    assert (jasterix.hasCategory("019"));
    assert (jasterix.hasEdition("019", "1.3"));
    jasterix.setEdition("019", "1.3");
    jasterix.setMapping("019", "");
